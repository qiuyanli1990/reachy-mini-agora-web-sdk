...
class IdleMicroMove(Move):  # type: ignore
    """Short expressive idle gesture around the current pose.

    The move eases from current pose to a small target offset, holds briefly,
    then returns to the captured start pose.
    """
    ...
    def __init__(
        ...
        ease_in_s: float = 0.35,
        hold_s: float = 0.25,
        ease_out_s: float = 0.45,
    ):
        self.start_head_pose = start_head_pose.copy()
        self.start_antennas = (float(start_antennas[0]), float(start_antennas[1]))
        self.start_body_yaw = float(start_body_yaw)
        self.target_head_pose = target_head_pose.copy()
        self.target_antennas = (float(target_antennas[0]), float(target_antennas[1]))
        self.target_body_yaw = float(target_body_yaw)
        self.ease_in_s = max(0.05, float(ease_in_s))
        self.hold_s = max(0.0, float(hold_s))
        self.ease_out_s = max(0.05, float(ease_out_s))
        self._duration = self.ease_in_s + self.hold_s + self.ease_out_s
    ...
    def evaluate(self, t: float) -> tuple[NDArray[np.float64] | None, NDArray[np.float64] | None, float | None]:
        if t <= self.ease_in_s:
            alpha = max(0.0, min(1.0, t / self.ease_in_s))
            head_pose = linear_pose_interpolation(self.start_head_pose, self.target_head_pose, alpha)
            antennas = np.array(
                [
                    self.start_antennas[0] + (self.target_antennas[0] - self.start_antennas[0]) * alpha,
                    self.start_antennas[1] + (self.target_antennas[1] - self.start_antennas[1]) * alpha,
                ],
                dtype=np.float64,
            )
            body = self.start_body_yaw + (self.target_body_yaw - self.start_body_yaw) * alpha
            return (head_pose, antennas, float(body))

        hold_end = self.ease_in_s + self.hold_s
        if t <= hold_end:
            return (
                self.target_head_pose.copy().astype(np.float64),
                np.array([self.target_antennas[0], self.target_antennas[1]], dtype=np.float64),
                float(self.target_body_yaw),
            )

        beta = max(0.0, min(1.0, (t - hold_end) / self.ease_out_s))
        head_pose = linear_pose_interpolation(self.target_head_pose, self.start_head_pose, beta)
        antennas = np.array(
            [
                self.target_antennas[0] + (self.start_antennas[0] - self.target_antennas[0]) * beta,
                self.target_antennas[1] + (self.start_antennas[1] - self.target_antennas[1]) * beta,
            ],
            dtype=np.float64,
        )
        body = self.target_body_yaw + (self.start_body_yaw - self.target_body_yaw) * beta
        return (head_pose, antennas, float(body))
...
class MovementManager:
    ...
    def __init__(
        ...
        self._idle_micro_min_interval_s = 3.0
        self._idle_micro_max_interval_s = 8.0
        self._idle_micro_first_delay_s = 2.0
        self._next_idle_micro_time = self._now() + self._idle_micro_first_delay_s
    ...
    def _schedule_next_idle_micro(self, current_time: float) -> None:
        interval = float(self._rng.uniform(self._idle_micro_min_interval_s, self._idle_micro_max_interval_s))
        self._next_idle_micro_time = current_time + interval

    def _build_idle_micro_move(self) -> IdleMicroMove:
        start_head, start_antennas, start_body = clone_full_body_pose(self._last_commanded_pose)

        # Small expressive offsets around current pose, in robot-friendly ranges.
        patterns = [
            {"roll": 11.0, "pitch": -4.0, "yaw": 7.0, "body": -4.0, "ant_l": 16.0, "ant_r": -6.0},
            {"roll": -11.0, "pitch": -4.0, "yaw": -7.0, "body": 4.0, "ant_l": 6.0, "ant_r": -16.0},
            {"roll": 5.0, "pitch": -8.0, "yaw": 14.0, "body": -6.0, "ant_l": 12.0, "ant_r": -3.0},
            {"roll": -5.0, "pitch": -8.0, "yaw": -14.0, "body": 6.0, "ant_l": 3.0, "ant_r": -12.0},
        ]
        pattern = patterns[int(self._rng.integers(0, len(patterns)))]
        ...
        target_head = compose_world_offset(start_head, offset_pose, reorthonormalize=True)
        target_antennas = (
            float(start_antennas[0] + np.deg2rad(pattern["ant_l"])),
            float(start_antennas[1] + np.deg2rad(pattern["ant_r"])),
        )
        target_body = float(start_body + np.deg2rad(pattern["body"]))

        return IdleMicroMove(
            start_head_pose=start_head,
            start_antennas=start_antennas,
            start_body_yaw=start_body,
    ...
    def _maybe_queue_idle_micro(self, current_time: float) -> None:
        """Interleave short expressive gestures while idling on breathing."""
        if self._is_listening:
            return
        if current_time < self._next_idle_micro_time:
            return
        if not isinstance(self.state.current_move, BreathingMove):
            return
        if self.move_queue:
            return
        idle_for = current_time - self.state.last_activity_time
        if idle_for < self._idle_micro_first_delay_s:
            return
        try:
            self.move_queue.append(self._build_idle_micro_move())
        finally:
            self._schedule_next_idle_micro(current_time)
    ...
    def _update_primary_motion(self, current_time: float) -> None:
        ...
        self._maybe_queue_idle_micro(current_time)
